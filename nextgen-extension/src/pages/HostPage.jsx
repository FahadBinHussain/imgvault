...
function summarizeActiveDownloadMessage(message = '') {
  ...
  if (/yt-dlp failed/i.test(raw)) {
    if (/RETRY_WITH_BEST_FORMAT/.test(raw)) {
      return raw;
    }
...
export default function HostPage() {
  ...
  useEffect(() => {
    ...
    const syncFromStorage = async () => {
      ...
      if (/\[RETRY_WITH_BEST_FORMAT\]/.test(activeRecord?.lastMessage || '')) {
    ...
    const handleStorageChange = (changes, areaName) => {
      ...
      if (/\[RETRY_WITH_BEST_FORMAT\]/.test(nextValue?.lastMessage || '')) {
        setRetryWithBest(true);
      }
    };
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(360px,0.8fr)] items-start">
          <div className="min-w-0 bg-base-100 border border-base-300 rounded-[var(--radius-box)] shadow-xl p-5 sm:p-6 space-y-4">
            ...
            <div className="rounded-[var(--radius-box)] border border-base-300 bg-base-200/60 p-4 space-y-3">
              ...
              {(activeNativeDownload?.requestId || activeDownloadRequestIdRef.current) && (
